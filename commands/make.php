    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        if ($partialName) {
            ...
            if(!is_file('partials/'.$fileName.'.php')){
                ...
                // Agregar la llamada a sajo_enqueue_partial_script al inicio
                $importScript = "<script src='<?= get_template_directory_uri(); ?>/js/partials/$fileName.js'></script>";
                file_put_contents('partials/'.$fileName.'.php', $importScript . $contents);
